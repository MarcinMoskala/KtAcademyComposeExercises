package kt.academy.advanced.navigation
...
fun Navigation() {
    ...
    NavHost(
            ...
            composable<ListScreenDestination> {
                ListScreen(
                    onOpenItemDetails = { courseId ->
                        controller.navigate(DetailScreenDestination(courseId))
                    }
                )
            }
            composable<DetailScreenDestination> { entry ->
                val args = entry.toRoute<DetailScreenDestination>()
                DetailScreen(
        ...
        popEnterTransition = {
            scaleIn(initialScale = .9f) + fadeIn(initialAlpha = .5f)
        },
        popExitTransition = {
